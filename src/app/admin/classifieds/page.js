...
export default function AdminClassifieds() {
  ...
  async function toggleHidden(id, current) {
    await supabase.from("classifieds").update({ is_hidden: !current }).eq("id", id);
    fetchAll();
  }

  async function toggleSold(id, current) {
    await supabase.from("classifieds").update({ is_sold: !current }).eq("id", id);
    fetchAll();
  }
    ...
    <main className="min-h-screen bg-neutral-950 text-white px-6 py-10">
      <div className="max-w-2xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-bold">Classifieds Moderation</h1>
            ...
            <div key={item.id} className="bg-neutral-900 border border-neutral-800 rounded-xl p-4">
              <div className="flex justify-between items-start">
                ...
                <div className="flex gap-2 text-xs">
                  ...
                  {item.is_sold && <span className="bg-neutral-700 text-neutral-300 px-2 py-1 rounded">SOLD</span>}
                </div>
              </div>
