...
export default function AdminFeatured() {
  ...
  const [mode, setMode] = useState("real"); // "real" or "demo"
  ...
  async function handleAdd(e) {
    ...
    const { error } = await supabase.from("featured_businesses").insert(payload);

    setSaving(false);

    if (error) {
      alert("Something went wrong. That business might already be featured.");
      return;
    }
    ...
    <main className="min-h-screen bg-neutral-950 text-white px-6 py-10">
      <div className="max-w-2xl mx-auto">
        ...
        {!loading && featured.length === 0 && (
          <p className="text-neutral-400">No featured entries yet.</p>
        )}
